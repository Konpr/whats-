...
def _extract_after_with_ranges(full: str, steps: int):
    ...
    for part in (ranges_txt or "").split(','):
        ...
        # Одиночный шаг разрешён (start == end)
...
class CollectSteps(lark.Visitor):
    ...
    def visit_scheduled(self, tree):
            ...
            direct_prompts = [
                ...
                if isinstance(c, _l.Tree) and getattr(c, "data", None) == "prompt"
            ]
            num_tok = next(
                (c for c in tree.children if isinstance(c, _l.Token) and c.type == "NUMBER"),
                None,
            )
            step_range_list = next(
                (c for c in tree.children if isinstance(c, _l.Tree) and getattr(c, "data", None) == "step_range_list"),
                None,
            )
            is_reverse = any(
                isinstance(c, _l.Tree) and getattr(c, "data", None) == "reverse_flag"
