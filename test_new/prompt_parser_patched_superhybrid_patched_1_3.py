...
def _build_ranges_schedules_from_components(
    ...
    *,
    ...
    Важно: эта функция НЕ парсит строку, а принимает уже извлечённые компоненты:
      - prompts: список промптов (уже разэкранированных)
      - ranges: список (start,end) шагов (уже приведённых к 1..steps)
      - steps: общее число шагов

    Цель — единая семантика для fast-path в get_schedule() и CollectSteps:
      - одинаковое сопоставление prompts ↔ ranges
      - одинаковая обработка reverse
      - одинаковая сборка префикса/суффикса и дырок (gap) как "пустой центр"
      - схлопывание подряд одинаковых сегментов

    Примечание: "пустой центр" задаётся empty_center (обычно "").
    """
    if steps <= 0:
        raise ValueError(f"steps must be positive, got {steps}")

    if not ranges:
        # если диапазонов нет — возвращаем один сегмент с пустым центром
        return [[int(steps), _concat_prefix_text_suffix(prefix, empty_center, suffix)]]
    ...
    for idx, (start, end) in enumerate(ranges):
        ...
        _append(min(end, steps), center)
        current = min(end + 1, steps + 1)
        if current > steps:
            break

    if current <= steps:
        _append(steps, empty_center)
