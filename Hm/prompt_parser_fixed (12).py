...
def _placeholderize_postfix_scheduled_blocks(text: str) -> str:
    ...
    # Сложность: O(N) суммарно.
    # Внутренний цикл (k от start до i) запускается только для непересекающихся
    # диапазонов скобок — их суммарная длина ≤ N, поэтому всё вместе O(N).
    while i < len(chars):
        ch = chars[i]

        # skip escaped char — проверяем границы чтобы не выйти за конец строки
        if ch == "\\":
        ...
        if ch == "]" and stack:
            start = stack.pop()
            # Only act on a closed top-level bracket block (no nested bracket depth)
            if not stack:
                j = i + 1
                # whitespace
                ...
                if j < len(chars) and chars[j] == ":":
                    ...
                    while j < len(chars) and str(chars[j]).isspace():
                        j += 1
                    # numeric boundary right after ]:
                    m = RE_NUMERIC.match("".join(chars[j:]))
                    if m:
                        # Replace commas in inner bracket content at top nesting only
                        depth_paren = 0
                        depth_brace = 0
                        depth_brack = 0
                        k = start + 1
...
def _strip_reverse_from_post(post: str, extra_rev: bool = False) -> tuple[str, bool]:
    ...
    if mrev:
        return post[mrev.end():], True
    mrev_s = RE_REVERSE_SUFFIX.search(post)
    if mrev_s:
        return post[:mrev_s.start()].rstrip(), True
    return post, False
...
class CollectSteps(lark.Visitor):
        ...
        if full.count('[') == 1 and full.count(']') == 1 and not _RE_UNESCAPED_ALT_OR_BANG.search(full):
            ...
            if m:
                ...
                try:
                    ...
                    prompts = _parse_inner_prompts(inner)
                    post, rev_flag = _strip_reverse_from_post(post, bool(rev_flag))
                    if rev_flag:
    ...
    def visit_start(self, tree):
        ...
        if m_ba and not _RE_UNESCAPED_ALT_OR_BANG.search(full) and full.count('[') == 1 and full.count(']') == 1:
            ...
            post, rev_flag = _strip_reverse_from_post(post or "", bool(rev_flag))
            prompts = _parse_inner_prompts(inner)
            # Реверс: применяем если reverse найден, независимо от наличия префикса/суффикса
            if rev_flag:
                prompts = list(reversed(prompts))
            boundary = _to_end_step(boundary_f, self.steps)
    ...
    def visit_scheduled(self, tree):
            ...
            num_tok = next(
                (c for c in tree.children if isinstance(c, lark.Token) and c.type == "NUMBER"),
                None,
            )
            ...
            # ── Boundary mode: "[...:N]" / "[...]:N" ─────────────────────────────
            boundary = _clamp(int(round(weight * self.steps)) if weight <= 1.0 else int(round(weight)), self.steps)
        ...
        if m_with_pre and prompt.count('[') == 1 and prompt.count(']') == 1:
            ...
            if re.search(r'\b\d+%?\s*-\s*\d+%?', post):
                ...
                try:
                    ...
                    inner_prompts = _parse_inner_prompts(inner)
                    post, rev_here = _strip_reverse_from_post(post or "", bool(rev_token))
...
def _parse_prompt_attention_impl(text):
    ...
    while i + 1 < len(res):
        ...
        # Нельзя "взвешивать" разделители/пустоту
        ...
        if j < len(res) and res[j][1] == 1.0:
            ...
            if m:
                ...
                try:
                    ...
                    if ATTENTION_DELTA_ADDITIVE:
                        res[i][1] = float(res[i][1]) + delta
                    else:
                        # не-аддитивный режим: дельта — это абсолютный вес
                        res[i][1] = delta
                except ValueError:
                    pass
                ...
                # continue без i+=1: позволяет "поймать" следующую дельту для того же токена
                continue
